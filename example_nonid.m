...
ftrue = f(x_true);
[ftilde_obs, gam_obs] = pairwise_align(y_train(1,:)',ftrue',tt', 0.01);
vv_obs = gam_to_v(gam_obs');

out = fdawarp(y_train',tt');
out = out.multiple_align_functions(y_train(1,:)',0.01);
gam_train = out.gam;
...
%% Fit Emulators
if bppr
    emu_ftilde = bpprPCA(x_train, ftilde_train', 4);
    emu_ftilde.plot()
    
    emu_vv = bpprPCA(x_train, vv_train, 4);
    emu_vv.plot()
else
    emu_ftilde = bassPCA(x_train, ftilde_train', 4);
    emu_ftilde.plot()
    
    emu_vv = bassPCA(x_train, vv_train, 4);
    emu_vv.plot()
end
...
if bppr
    model_ftilde = ModelBpprPca_func(emu_ftilde, input_names);
    model_vv = ModelBpprPca_func(emu_vv, input_names);
else
    model_ftilde = ModelBassPca_func(emu_ftilde, input_names);
    model_vv = ModelBassPca_func(emu_vv, input_names);
end
setup.addVecExpperiments(ftilde_obs', model_ftilde, 0.01, 20, ones(1,nt));
setup.addVecExpperiments(vv_obs', model_vv, 0.01, 20, ones(1,nt));
setup.setTemperatureLadder(1.05.^(0:3));
...
pair_plot(parmat_array, input_names);
...
for idx = expnums
    ...
    plot(time_new, ftrue, 'k', 'Linewidth', 2)
