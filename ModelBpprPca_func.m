classdef ModelBpprPca_func < handle
    ...
    methods
        function obj = ModelBpprPca_func(bmod, input_names, exp_ind, s2)
            % **PCA Based Model Emulator using BPPR**
            %
            % This function setups up emulator object
            %
            % bmod: a object of the type bpprBasis
            ...
            obj.mod = bmod;
            obj.stochastic = true;
            obj.nmcmc = length(bmod.bm_list{1}.samples.sdResid);
            obj.input_names = input_names;
            obj.basis = obj.mod.basis;
            obj.meas_error_cor = eye(size(obj.basis,1));
            obj.discrep_cov = eye(size(obj.basis,1))*1e-12;
            obj.ii = 1;
            npc = obj.mod.nbasis;
            if npc > 1
                obj.trunc_error_var = diag(cov(obj.mod.trunc_error'));
            else
                obj.trunc_error_var = diag(reshape(cov(obj.mod.trunc_error'),1,1));
            end
            obj.mod_s2 = zeros(obj.nmcmc, npc);
            for i = 1:npc
                obj.mod_s2(:,i) = obj.mod.bm_list{i}.samples.sdResid.^2;
            end
        ...
        function pred = eval(obj, parmat, pool, nugget)
            ...
            if pool
                pred = obj.mod.predict(parmat_array, obj.ii, nugget);
            else
                keyboard
            end
